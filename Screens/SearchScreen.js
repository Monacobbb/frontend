...
const SearchScreen = ({ navigation, route }) => {
    ...
    useEffect(() => {
        const fetchRecentSearches = async () => {
            try {
                const response = await axios.post('http://192.168.200.142:8080/posts/recentSearch');
    ...
    const handleSearch = async () => {
        if (searchText) {
            try {
                const response = await axios.get(`http://192.168.200.142:8080/posts/search/${searchText}`);
